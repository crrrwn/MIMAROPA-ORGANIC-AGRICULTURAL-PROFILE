...
export default function EncoderLogin() {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await signIn(email, password, rememberMe, 'encoder', decodeURIComponent(province));
      navigate('/dashboard');
    } catch (err) {
        ...
        {showForgot ? (
            ...
            {forgotSent ? (
              ...
              <form onSubmit={handleForgotPassword}>
                <input
                  ...
                  className="w-full px-4 py-3 border border-oa-green/40 rounded-lg mb-4"
                  required
                />
                {error && <p className="text-red-600 text-sm mb-2">{error}</p>}
                <button type="submit" className="w-full py-3 bg-oa-blue text-white rounded-lg font-medium">Send Reset Link</button>
              </form>
            )}
            <button onClick={() => { setShowForgot(false); setForgotSent(false); }} className="mt-4 text-sm text-oa-brown">← Back to Login</button>
          </>
        ) : (
          <form onSubmit={handleSubmit}>
            ...
            <div className="flex items-center justify-between mb-4">
              ...
              <button type="button" onClick={() => setShowForgot(true)} className="text-sm text-oa-blue hover:underline">Forgot Password?</button>
            </div>
            {error && <p className="text-red-600 text-sm mb-2">{error}</p>}
            ...
            <p className="mt-4 text-center text-sm text-oa-brown">
              Don't have an account? <Link to={`/encoder/${province}/register`} className="text-oa-blue font-medium hover:underline">Register</Link>
            </p>
          </form>
        )}
      </div>
