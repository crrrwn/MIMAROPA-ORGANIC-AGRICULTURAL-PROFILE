...
export default function AdminLogin() {
  ...
  return (
    <SharedAuthLayout title="Admin Login" backHref="/" hideBack={showForgot} backIconOnly>
        ...
        {showForgot ? (
          <div key="forgot" className="rounded-xl bg-palette-cream/50 border border-palette-sky/40 p-4">
            ...
            <p className="text-xs text-palette-brown/90 mb-4">
              Enter the email address linked to your admin account. We’ll send instructions to reset your password.
            </p>
            {forgotSent ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <button
                type="submit"
                disabled={loading}
                className="w-full py-2.5 text-sm bg-palette-green text-white rounded-xl font-semibold shadow-sm disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2 min-h-[40px] hover:opacity-90"
              >
                {loading ? (
                  <>
